...
const DOC = 'state';
const PIN = String(process.env.JUDGE_PIN || '2604').trim();   /* .trim() — от случайного пробела при вставке */
...
/* сравнение без утечки по времени: хеши всегда одной длины, исключений быть не может */
...
const isJudge = req => {
  const h = req.headers['x-judge-pin'];
  return same(String(Array.isArray(h) ? h[0] : (h || '')).trim(), PIN);
};
